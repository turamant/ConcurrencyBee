...
async def create_table():
    async with aiosqlite.connect('my_weather.db') as db:
    ...
    async with ClientSession() as session:
        ...
        async with session.get(url=url, params=params) as response:
            ...
            try:
                ...
                clouds = weather_json["weather"][0]["main"]
                return clouds
            except KeyError:
                return 'Нет данных'


async def get_translate(text, source, target):
...
async def handle(request):
    ...
    city_en = await get_translate(city_ru, 'ru', 'en')
    weather_en = await get_weather(city_en)
    weather_ru = await get_translate(weather_en, 'en', 'ru')
